...
def generate2optNeighbours(tour):
    ...
    for i in range(2,cities-1):
        for j in range(1,cities-i+1) :
            next_tour = tour[:]
            next_tour[j:j+i] = reversed(next_tour[j:j+i])
            all_possible_neighbours.append(next_tour)
...
def generate3optNeighbours(tour):
    ...
    for i in range(cities) :
        ...
        if i == 0 :
            ...
            lim = cities
        for j in range(i+2,lim) :
            for k in range(j+2,lim) :
                tour1 = tour[:]
                tour1[i+1:j+1] = reversed(tour1[i+1:j+1])
                tour1[j+1:k+1] = reversed(tour1[j+1:k+1])
                all_possible_neighbours.append(tour1)
                
                tour2 = tour[:i+1] + tour[j+1:k+1] + tour[i+1:j+1] + tour[k+1:]
                all_possible_neighbours.append(tour2)

                tour3 = tour[:]
                tour3[i+1:j+1] = reversed(tour3[i+1:j+1])
                tour3 = tour3[:i+1] + tour3[j+1:k+1] + tour3[i+1:j+1] + tour3[k+1:]
                all_possible_neighbours.append(tour3)

                tour4 = tour[:]
                tour4[j+1:k+1] = reversed(tour4[j+1:k+1])
                tour4 = tour4[:i+1] + tour4[j+1:k+1] + tour4[i+1:j+1] + tour4[k+1:]
                all_possible_neighbours.append(tour4)
...
def hillClimbUtil(tour,getNeighbours) :
    next_list = getNeighbours(tour)
    prev_length = getTourLength(next_list[0])
    prev_tour = next_list[0]

    for el in next_list :
        length = getTourLength(el)
        if length < prev_length :
            prev_length = length
            prev_tour = el

    return prev_tour,prev_length
...
def hillClimbFull(initial_tour, getNeighbours):
    ...
    prev_length = getTourLength(initial_tour)
    prev_tour = initial_tour[:]
    while True :
        tour,length = hillClimbUtil(prev_tour,getNeighbours)
        if length < prev_length :
            prev_length = length
            prev_tour = tour
            tourLengthList.append(length)
        else :
            tourLengthList.append(prev_length)
            break
    minTour = prev_tour
...
def nearestNeighbourTour(initial_city):
    ...
    previous_city = initial_city
    for i in range(1,cities) :
        nearest_city = 0
        nearest_length = float('Inf')
        for j in range(cities) :
            if visited[j] == 0 :
                length = getDistance(nodeDict[previous_city],nodeDict[j+1])
                if length < nearest_length :
                    nearest_length = length
                    nearest_city = j+1
        visited[nearest_city-1] = 1
...
def finalOrder(mst, initial_city):
    ...
    def preorder(city) :
        ...
        next_list = []
        for el in mst :
            if el[0] == city and visited[el[1]-1] == 0:
                next_list.append(el[1])
            elif el[1] == city and visited[el[0]-1] == 0:
                next_list.append(el[0])
        for el in next_list :
            visited[el-1] = 1
            preorder(el)
